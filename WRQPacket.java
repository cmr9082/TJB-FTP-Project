...
public class WRQPacket implements TFTPConstants {
   // Attributes 
   private int opcode;
   ...
   */
   private WRQPacket(InetAddress _toAddress, int _opcode,  String _filename, String _mode, int _port ) {
      opcode = _opcode;
   ...
   */
   private DatagramPacket build() {
      ...
      DatagramPacket WRQpacket = new DatagramPacket(baos.toByteArray(), 0, (baos.toByteArray()).length, toAddress, port);
   ...
   */
   public void dissect(DatagramPacket  WRQpacket) {
      // Getting the packet's address and port
      toAddress =  WRQpacket.getAddress();
      port =  WRQpacket.getPort();
      
      // Creating input stream to read the packet data
      ByteArrayInputStream bias = new ByteArrayInputStream( WRQpacket.getData(), WRQpacket.getOffset(),  WRQpacket.getLength());
      ...
      try {
         ...
         System.out.println("ERROR while dissecting WRQ Packet " + ex);
      }
